...
TOPIC_NAMES = ["Biology", "Chemistry", "Physics", "Mathematics", "CS Fundamentals", "AI & ML"]
MASTERY_THRESHOLD = 0.85
MAX_STEPS = 80


class StudentProfile:
    """Fixed student characteristics sampled at episode start."""
    def __init__(self, rng: np.random.Generator):
        self.learning_speed     = rng.uniform(0.05, 0.30)
        self.prior_knowledge    = rng.uniform(0.0,  0.35)
        self.attention_span     = rng.uniform(0.4,  1.0)
        self.quiz_anxiety       = rng.uniform(0.0,  0.5)
        self.curiosity          = rng.uniform(0.3,  1.0)
        self.preferred_modality = rng.integers(0, 3)
    ...
    def to_array(self) -> np.ndarray:
        return np.array([
            ...
            self.quiz_anxiety,
            self.curiosity,
            float(self.preferred_modality) / 2.0,
        ], dtype=np.float32)
...
class MalamiEnv(gym.Env):
    ...
    Observation (21 dims):
      ...
      [9-14] per-topic masteries
      [15-20] student profile features
    """
    ...
    def reset(self, *, seed=None, options=None):
        ...
        self.profile = (
            ...
            else StudentProfile(self._np_rng)
        )
        ...
        self.hint_requests     = 0
        self.step_count        = 0
        return self._get_obs(), {"topics_completed": 0, "current_topic": TOPIC_NAMES[self.current_topic]}
    ...
    def step(self, action: int):
        ...
        info.update({
            ...
            "current_topic": TOPIC_NAMES[self.current_topic],
